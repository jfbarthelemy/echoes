settings.outformat = "html";
settings.render = -1;

// settings.outformat ="svg";
// settings.render = 0;
// // Hint: render svg format and then convert to png by "magick -density 300 file.svg file.png"

// settings.outformat = "pdf";
// settings.render = 8;
// settings.prc = true;

import graph3;
import palette;
import three;

srand(1949);

size(200,300,keepAspect=true);

currentprojection=orthographic(10,10,10);
currentlight.background=black+opacity(0.0);

typedef triple surffunc(pair);

surffunc gensphfunc(real r) {
return new triple(pair t) {return (r*sin(t.x)*cos(t.y),r*sin(t.x)*sin(t.y),r*cos(t.x));};
}

surffunc genspherofunc(real c, real q) {
return new triple(pair t) {return (c*sqrt(q**2-1)*sin(t.x)*cos(t.y),c*sqrt(q**2-1)*sin(t.x)*sin(t.y),c*q*cos(t.x));};
}

// surface s=surface(gensphfunc(1),(0,0),(pi,2pi),64,128,Spline);
// // s.colors(palette(s.map(zpart),Rainbow()+opacity(0.5)));
// draw(s,lightred+opacity(0.2),render(merge=true));

for(int i=1; i <= 4; ++i) {
    surface s=surface(genspherofunc(1,1+i/10),(0,0),(pi,2pi),32,64,Spline);
    draw(s,rgb(unitrand(),unitrand(),unitrand())+opacity(1-(i-1)/6),render(merge=true));
}

// for(int i=2; i <= 5; ++i) {
//     surface s=surface(gensphfunc(i/5),(0,0),(pi,2pi),32,64,Spline);
//     draw(s,rgb(unitrand(),unitrand(),unitrand())+opacity(1-(i-2)/6),render(merge=true));
// }



// real x(real t) {return a*cos(pi*t);}
// real y(real t) {return b*sin(pi*t);}
// real z(real t) {return 0;}
// path3 p=graph(x,y,z,0,0.5,operator ..);
// draw(p,heavyred+dashed);

// real x(real t) {return a*cos(pi*t);}
// real y(real t) {return 0;}
// real z(real t) {return c*sin(pi*t);}
// path3 p=graph(x,y,z,0,0.5,operator ..);
// draw(p,heavyred+dashed);

// real x(real t) {return 0;}
// real y(real t) {return b*cos(pi*t);}
// real z(real t) {return c*sin(pi*t);}
// path3 p=graph(x,y,z,0,0.5,operator ..);
// draw(p,heavyred+dashed);

// draw((0,0,0)--(0.3,0,0),royalblue,Arrow3);
// draw((0,0,0)--(0,0.3,0),royalblue,Arrow3);
// draw((0,0,0)--(0,0,0.3),royalblue,Arrow3);

// draw((0,0,0)--(a,0,0),heavyred+dashed);
// draw((0,0,0)--(0,b,0),heavyred+dashed);
// draw((0,0,0)--(0,0,c),heavyred+dashed);

// label(Label("\scriptsize $a$"),(a,0,0),N,black);
// label(Label("\scriptsize $b=\eta\,a$"),(0,b,0),E,black);
// label(Label("\scriptsize $c=\omega\,a$"),(0,0,c),NE,black);
