...
currentprojection=orthographic(0,-10,1);
...
draw(sphere,black+opacity(0.3),render(merge=true));

triple [] lp ;

bool notinter(triple p, triple[] lp, real R) {
    for (triple pt: lp) {
        real d = length(pt-p);
        if (d <= 5*R) {
            return false;
...
for(int i=0; i < 200; ++i) {
    ...
    if (notinter(c, lp, Rp)) {
        draw(ell,heavyblue+opacity(1),render(merge=true));
        lp.push(c);
    }
}
