...
$$
\trans{\uu{A}}\cdot\uu{A}=a^2 \uv{e}^{\uu{A}}_1\otimes\uv{e}^{\uu{A}}_1 + b^2 \uv{e}^{\uu{A}}_2\otimes\uv{e}^{\uu{A}}_2 + c^2 \uv{e}^{\uu{A}}_3\otimes\uv{e}^{\uu{A}}_3 = \sum_{i=1}^3 \rho_i \uv{e}^{\uu{A}}_i\otimes\uv{e}^{\uu{A}}_i
$${#eq-ellipsoid}
...
$$
\uu{I}^{\uu{A}}=I_1\,\uv{e}^{\uu{A}}_1\otimes\uv{e}^{\uu{A}}_1 + I_2\,\uv{e}^{\uu{A}}_2\otimes\uv{e}^{\uu{A}}_2 + I_3\,\uv{e}^{\uu{A}}_3\otimes\uv{e}^{\uu{A}}_3 = \sum_{i=1}^3 I_i\,\uv{e}^{\uu{A}}_i\otimes\uv{e}^{\uu{A}}_i
$${#eq-decI}

The components $I_i$ can be obtained by identification with expressions derived from the Newtonian potential ([@kellogg1929], [@eshelby1957]). They are gathered in @tbl-Ii or recalled a little later in a generic form ([@barthelemyIJSS2016], [@barthelemyIJES2020_hilltrans]).
...
Before detailing the components of $\uuuu{U}^{\uu{A}}$ and $\uuuu{V}^{\uu{A}}$ it is convenient to introduce the coefficients $I_{ij}$ in @tbl-Iij adapted from the approach proposed in [@eshelby1957].
...
Components $I_i$ and $I_{ij}$ are alternatively given in a generic form in the following section to be expanded.

::: {.callout-note collapse=true}

## Generic expressions of $I_i$ and $I_{ij}$ (click for details)
